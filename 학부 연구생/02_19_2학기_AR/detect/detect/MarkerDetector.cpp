...
{
	//cv::Mat bgraMat(frame.rows, frame.cols, CV_8UC3);
	...
	for (auto i = 0; i < detectedMarkers.size(); i++) {
		cv::Mat markerImage = frame.clone();
		Marker & m = detectedMarkers[i];
		m.drawContour(markerImage);
		//cv::Mat markerSubImage = markerImage(cv::boundingRect(m.points));

		char id_text[1024];
		_itoa_s(m.id, id_text, 10);

		int center_x = int(markerImage.rows / 2.0);
		int center_y = int(markerImage.cols / 2.0);

		int thickness = 2;
		cv::Point location(center_x, center_y);
		int font = CV_FONT_HERSHEY_SCRIPT_SIMPLEX;// hand-writing style font
		double fontScale = 1.5;
		putText(markerImage, id_text, location, font, fontScale, cv::Scalar(0,255,255), thickness);
		//////////////////
		cv::imshow("colorEdgeImage", markerImage);
	}
...
void MarkerDetector::recognizeMarkers(const cv::Mat& grayscale, std::vector<Marker>& detectedMarkers)
{
	...
	for (size_t i = 0; i < detectedMarkers.size(); i++)
	{
		...
		cv::imshow("edgeImage", markerImage);
