...
int MouseX = 500, MouseY = gHeight / 2;
int mX = 0, mY = 0;
...
int spot[15][4] = {
	...
	//{ 850, 200, 915, 320 },{ 850, 330, 915, 475 },
	//{ 850, 480, 915, 605 },{ 850, 610, 915, 760 },
	...
	for (int i = 0; i < 15; i++)
	{
		...
		if (isUsing[i] >= 2)
			if (MouseX > spot[i][0] - resize * 1.5 && MouseX < spot[i][2] + resize * 1.5 && MouseY > spot[i][1] - resize && MouseY < spot[i][3] + resize) {
				...
				sprintf(sendData, "%d, %d, %d, %d, %d, %d, %d, %d, %d", 0, i, pressedSpace[i], 0, 0, 0, 0, 0, 0);
				CommSend(sendData);
				if (pressedSpace[i] > 3)
					pressedSpace[i] = 0;
			}
...
BOOL _GameMode(HINSTANCE hInstance, int nCmdShow, int x, int y, int bpp)
{
	...
	wc.lpszClassName = "SECONDPLAYER";
	RegisterClass(&wc);


	if (gFullScreen) {
		if ((MainHwnd = CreateWindowEx(0, "SECONDPLAYER", NULL, WS_POPUP, 0, 0, GetSystemMetrics(SM_CXSCREEN),
			GetSystemMetrics(SM_CYSCREEN), NULL, NULL, hInstance, NULL)) == NULL)
			ExitProcess(1);
	}
	else {
		if ((MainHwnd = CreateWindow("SECONDPLAYER", "SECONDPLAYER", WS_OVERLAPPEDWINDOW, 0, 0, x,
...
void CALLBACK _GameProc(HWND hWnd, UINT message, UINT wParam, DWORD lParam)
{
	if (timer < ending-1) {
		...
		if (pressedSpace[14]) {
			...
			//sprintf(sendData, "%d, %d, %d, %d, %d, %d, %d, %d, %d", 4, isUsing[13], isUsing[12], isUsing[11], 0, 0, 0, 0, 0);
			//CommSend(sendData);
		}*/
		/*if (pressedSpace[14]) {
			BackScreen->BltFast((spot[14][0] + spot[14][2]) / 2, (spot[14][1] + spot[14][3]) / 2, pop_one, &popPlayerRect, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
		...
		if (lookDir == 1) {
			...
			sprintf(sendData, "%d, %d, %d, %d, %d, %d, %d, %d, %d", 1, SpriteRect.left, SpriteRect.top, SpriteRect.right, SpriteRect.bottom, MouseX - 48, MouseY - 48, 0, 1);
			CommSend(sendData);
		}
		...
		if (lookDir == 2) {
			...
			sprintf(sendData, "%d, %d, %d, %d, %d, %d, %d, %d, %d", 1, SpriteRect.left, SpriteRect.top, SpriteRect.right, SpriteRect.bottom, MouseX - 48, MouseY - 48, 0, 2);
			CommSend(sendData);
		}

		/*
		for (int i = 0; i < 15; i++) {
			sprintf(sendData, "%d, %d, %d, %d, %d, %d, %d, %d, %d", 0, i, pressedSpace[i], 0, 0, 0, 0, 0, 0, 0);
			CommSend(sendData);
		}*/
		///////////////////
	}
...
void _GameProcDraw(char *recvData)
{
	...
	int type;
		...
		if (type == 1) {
			...
			if (Display.right >= 0 && Display.right < 3)
				BackScreen->BltFast(Display.left, Display.top, Npc_1, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right >= 3 && Display.right < 7)
				BackScreen->BltFast(Display.left, Display.top, Npc_2, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right >= 7 && Display.right < 11)
				BackScreen->BltFast(Display.left, Display.top, Npc_3, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);

			isUsing[Display.right] = Display.bottom;
		} // pop
		else if (type == 3) {
			if (Display.right == 1) // sleep
				BackScreen->BltFast(Display.left, Display.top, pop_sleep, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 2) // good 
				BackScreen->BltFast(Display.left, Display.top, pop_good, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 3) // coin
				BackScreen->BltFast(Display.left, Display.top, pop_coin, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 4) // clean
				BackScreen->BltFast(Display.left, Display.top, pop_clean, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 5) // wait
				BackScreen->BltFast(Display.left, Display.top, pop_wait, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 6) // meat
				BackScreen->BltFast(Display.left, Display.top, pop_meat, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 7) // item
				BackScreen->BltFast(Display.left, Display.top, pop_item, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 8) // one
				BackScreen->BltFast(Display.left, Display.top, pop_one, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 9) // two
				BackScreen->BltFast(Display.left, Display.top, pop_two, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 10) // three
				BackScreen->BltFast(Display.left, Display.top, pop_three, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
			else if (Display.right == 11) // four
				BackScreen->BltFast(Display.left, Display.top, pop_four, &Sprite, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
		} // 음식
		else if (type == 4) {
			isUsing[13] = Sprite.left;
			isUsing[12] = Sprite.top;
			isUsing[11] = Sprite.right;
			//isUsing[3] = Sprite.bottom;
			//isUsing[4] = Display.left;
			//isUsing[5] = Display.top;
			//isUsing[6] = Display.right;
		}
		else if (type == 10)
			score = Sprite.left;

		count++;
		if (count == 30) {// 8  // 15  // 24  // 25
			if (gFullScreen)
				RealScreen->Flip(NULL, DDFLIP_WAIT);
			else {
				GetWindowRect(MainHwnd, &WinRect);
...
int PASCAL WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	//player
	SpriteImage = DDLoadBitmap(DirectOBJ, "DinoSprites2_2P.BMP", 0, 0);
	SpriteImage_D = DDLoadBitmap(DirectOBJ, "DinoSprites2F_2P.BMP", 0, 0);


	SpriteImage_other = DDLoadBitmap(DirectOBJ, "DinoSprites2_1P.BMP", 0, 0);
	SpriteImage_D_other = DDLoadBitmap(DirectOBJ, "DinoSprites2F_1P.BMP", 0, 0);
